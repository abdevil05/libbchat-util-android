...
import network.loki.messenger.libbchat_util.pro.ProConfig
import network.loki.messenger.libbchat_util.protocol.ProProfileFeatures
import network.loki.messenger.libbchat_util.util.BaseCommunityInfo
import network.loki.messenger.libbchat_util.util.BlindedContact
import network.loki.messenger.libbchat_util.util.ConfigPush
import network.loki.messenger.libbchat_util.util.Contact
import network.loki.messenger.libbchat_util.util.Conversation
import network.loki.messenger.libbchat_util.util.ExpiryMode
import network.loki.messenger.libbchat_util.util.GroupInfo
import network.loki.messenger.libbchat_util.util.GroupMember
import network.loki.messenger.libbchat_util.util.UserPic
...
sealed class Config(initialPointer: Long): LibBChatUtilCApi() {
...
interface ReadableConversationVolatileConfig: ReadableConfig {
    ...
    fun getClosedGroup(bchatId: String): Conversation.ClosedGroup?
...
interface MutableConversationVolatileConfig : ReadableConversationVolatileConfig, MutableConfig {
    ...
    fun getOrConstructClosedGroup(bchatId: String): Conversation.ClosedGroup
    fun eraseClosedGroup(bchatId: String): Boolean
...
interface ReadableGroupKeysConfig {
    ...
    fun supplementFor(userBChatIds: List<String>): ByteArray
    ...
    fun getSubAccountToken(bchatId: String, canWrite: Boolean = true, canDelete: Boolean = false): ByteArray
    fun currentGeneration(): Int
    fun size(): Int
}

interface MutableGroupKeysConfig : ReadableGroupKeysConfig {
    fun makeSubAccount(bchatId: String, canWrite: Boolean = true, canDelete: Boolean = false): ByteArray
