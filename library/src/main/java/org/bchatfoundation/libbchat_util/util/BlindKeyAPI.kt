...
import org.bchatfoundation.libbchat_util.LibBChatUtilCApi

object BlindKeyAPI : LibBChatUtilCApi() {
    ...
    /**
     * Takes in a standard bchatId and returns a flag indicating whether it matches the given
     * blindedId for a given serverPubKey
     *
     * @param bchatId The bchat ID to check, 66bytes string
     * @param blindedId The blinded ID to check against, 66bytes string
     * @param serverPubKey The server public key, 64bytes string
     */
    external fun bchatIdMatchesBlindedId(
        bchatId: String,
        blindedId: String,
        serverPubKey: String,
    ): Boolean
