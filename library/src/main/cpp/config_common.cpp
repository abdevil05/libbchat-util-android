...
Java_network_loki_messenger_libbchat_1util_ConfigKt_createConfigObject(
        JNIEnv *env,
        jclass _clazz,
    ...
    return jni_utils::run_catching_cxx_exception_or_throws<jlong>(env, [=] {
        ...
        if (config_name.view() == "Contacts") {
            return reinterpret_cast<jlong>(new bchat::config::Contacts(secret_key.get(), initial));
        } else if (config_name.view() == "UserProfile") {
            return reinterpret_cast<jlong>(new bchat::config::UserProfile(secret_key.get(), initial));
        } else if (config_name.view() == "UserGroups") {
            return reinterpret_cast<jlong>(new bchat::config::UserGroups(secret_key.get(), initial));
        } else if (config_name.view() == "ConvoInfoVolatile") {
            return reinterpret_cast<jlong>(new bchat::config::ConvoInfoVolatile(secret_key.get(), initial));
        } else {
