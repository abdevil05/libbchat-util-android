...
inline auto ptrToProfile(JNIEnv* env, jobject obj) {
    return dynamic_cast<bchat::config::UserProfile *>(ptrToConfigBase(env, obj));
}
...
Java_org_bchatfoundation_libbchat_1util_UserProfile_getNtsExpiry(JNIEnv *env, jobject thiz) {
    ...
    if (nts_expiry == std::nullopt) {
        auto expiry = util::serialize_expiry(env, bchat::config::expiration_mode::none,
                                             std::chrono::seconds(0));
        return expiry.release();
    }
    auto expiry = util::serialize_expiry(env, bchat::config::expiration_mode::after_send,
...
Java_org_bchatfoundation_libbchat_1util_UserProfile_setProConfig(JNIEnv *env, jobject thiz,
    ...
    jni_utils::run_catching_cxx_exception_or_throws<void>(env, [=]() {
        ...
        auto r = key_ref.get();
        bchat::cleared_uc64 rotating_privkey;
