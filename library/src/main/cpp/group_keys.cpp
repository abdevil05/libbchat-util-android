...
inline auto ptrToKeys(JNIEnv* env, jobject obj) {
    return dynamic_cast<bchat::config::groups::Keys*>(ptrToConfigSig(env, obj));
}
...
Java_org_bchatfoundation_libbchat_1util_GroupKeysConfig_00024Companion_newInstance(JNIEnv *env,
    ...
    return jni_utils::run_catching_cxx_exception_or_throws<jlong>(env, [=] {
        ...
        auto info = reinterpret_cast<bchat::config::groups::Info*>(info_pointer);
        auto members = reinterpret_cast<bchat::config::groups::Members*>(members_pointer);

        auto* keys = new bchat::config::groups::Keys(user_key_bytes,
                                                       pub_key_bytes,
                                                       secret_key_optional,
...
Java_org_bchatfoundation_libbchat_1util_GroupKeysConfig_loadKey(JNIEnv *env, jobject thiz,
    ...
    auto info = reinterpret_cast<bchat::config::groups::Info*>(info_ptr);
    auto members = reinterpret_cast<bchat::config::groups::Members*>(members_ptr);
...
Java_org_bchatfoundation_libbchat_1util_GroupKeysConfig_decrypt(JNIEnv *env, jobject thiz,
    ...
    return jni_utils::run_catching_cxx_exception_or_throws<jobject>(env, [=] {
        ...
        auto sender_bchat_id = jni_utils::jstring_from_optional(env, sender.data());

        return jni_utils::new_kotlin_pair(env, plaintext_bytes.get(), sender_bchat_id.get());
    });
}
...
Java_org_bchatfoundation_libbchat_1util_GroupKeysConfig_makeSubAccount(JNIEnv *env,
    ...
    return jni_utils::run_catching_cxx_exception_or_throws<jbyteArray>(env, [=] {
        auto new_subaccount_key = ptrToKeys(env, thiz)->swarm_make_subaccount(
                jni_utils::JavaStringRef(env, bchat_id).view(), can_write, can_delete);
...
Java_org_bchatfoundation_libbchat_1util_GroupKeysConfig_supplementFor(JNIEnv *env,
    ...
    return jni_utils::run_catching_cxx_exception_or_throws<jbyteArray>(env, [=] {
        auto ptr = ptrToKeys(env, thiz);
        std::vector<std::string> user_bchat_ids;
        for (int i = 0, size = env->GetArrayLength(j_user_bchat_ids); i < size; i++) {
            jni_utils::JavaLocalRef element(
                    env, (jstring)(env->GetObjectArrayElement(j_user_bchat_ids, i)));

            user_bchat_ids.emplace_back(jni_utils::JavaStringRef(env, element.get()).view());
        }

        auto supplement = ptr->key_supplement(user_bchat_ids);
